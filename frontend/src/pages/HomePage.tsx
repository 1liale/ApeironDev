          ...
          >
            <div className="flex flex-col h-full">
              <div className="p-1.5 border-b border-border flex items-center bg-muted flex-shrink-0 h-[41px]">
                <Button variant="ghost" size="icon" onClick={toggleSidebar} title={isSidebarCollapsed ? "Open Files" : "Close Files"}>
                  {isSidebarCollapsed ? (
                ...
                <span className="ml-2 text-sm text-muted-foreground truncate">
                  {activeFile || (isSidebarCollapsed && !activeFile ? "Select a file" : "No file selected")}
                </span>
              </div>
              <div className="flex-grow overflow-auto">
                <EditorPanel activeFile={activeFile} isDark={isDark} initialContent={`# Welcome to ${activeFile || "new_file.py"}\nprint("Hello, world!")`} />
              </div>
            </div>
          </ResizablePanel>
          <ResizableHandle withHandle />
